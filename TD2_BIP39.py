...
print("\nEntropie : ", bin(int.from_bytes(entropy, byteorder=sys.byteorder))[2:])
...
print("\nQuatre premiers bits du SHA256 de l'entropie : ", quatre_bits_string)
print("\nCombinaison des deux : ", combined_bits)

split_combined_bits = []
for i in range(12):
    split_combined_bits.append(combined_bits[i*11:11*i+11])

print("\nDivision en séries de 11 bits : ", split_combined_bits)

bip_file = open("./english.txt",'r')
data = bip_file.read()
data = data.split('\n')

print("\nLes douzes mots selon la liste BIP39 :")
seed = []
